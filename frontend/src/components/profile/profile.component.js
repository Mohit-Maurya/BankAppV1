            ...
            <div className="card mx-auto mt-5" style={{ width: '50vw' }}>
                ...
                <div className="card-body">
                    <div className="col-6 form-group mb-3">
                        ...
                        {update ? <input type="text" className="form-control" id="name" defaultValue={data.name} onChange={(e) => setData((prevState) => ({ ...prevState, name: e.target.value }))} disabled={!update} /> :
                            <p className='ms-3'> {data.name}</p>}
                    </div>
                    <div className='row'>
                        <div className="col-6 form-group mb-3">
                            <label><b>PanCard Number </b></label>
                            {update ? <input type="text" className="form-control" id="pancard" defaultValue={data.pan} onChange={(e) => setData((prevState) => ({ ...prevState, pan: e.target.value }))} disabled={!update} /> :
                                <p className='ms-3'>  {data.pan}</p>}
                        </div>
                        <div className="col-6 form-group mb-3">
                            <label><b>Aadhaar Number</b></label>
                            {update ? <input type="text" className="form-control" id="aadhaar" defaultValue={data.aadhaarNumber} onChange={(e) => setData((prevState) => ({ ...prevState, aadhaarNumber: e.target.value }))} disabled={!update} /> :
                    ...
                    <div className='row'>
                        ...
                        <div className="col-6 form-group mb-3">
                            <label><IoMail className='mb-1'/> <b> Email </b></label>
                            {update ? <input type="text" className="form-control" id="email" defaultValue={data.email} onChange={(e) => setData((prevState) => ({ ...prevState, email: e.target.value }))} disabled={!update} />
                                : <p className='ms-3'> {data.email}</p>}
                        </div>
                    </div>
                    <div className="col-11 form-group mb-3">
                        <label><IoHome className='mb-1'/> <b> Permanent Address </b> </label>
                        {update ? <input type="text" className="form-control" id="permanentAddress" defaultValue={data.permanentAddress} onChange={(e) => setData((prevState) => ({ ...prevState, permanentAddress: e.target.value }))} disabled={!update} />
                                 : <p className='ms-3'> {data.permanentAddress}</p>}
                    </div>
                    <div className="col-11 form-group mb-3">
                        <label><IoHome className='mb-1'/> <b> Temporary Address </b> </label>
                        {update ? <input type="text" className="form-control" id="temporaryAddress" defaultValue={data.temporaryAddress} onChange={(e) => setData((prevState) => ({ ...prevState, temporaryAddress: e.target.value }))} disabled={!update} />
